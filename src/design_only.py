...
class HBTF(pyc.Cycle):
    ...
    def setup(self):
        ...
        if design:
            balance.add_balance("W", units="lbm/s", eq_units="lbf", lower=300, val=1000)
            ...
            balance.add_balance("BPR", lower=2.0, upper=14.0, eq_units="inch**2")
            self.connect("balance.BPR", "splitter.BPR")
            self.connect("byp_nozz.Throat:stat:area", "balance.lhs:BPR")
...
class MPhbtf(pyc.MPCycle):
    ...
    def setup(self):
        ...
        self.od_pts = ["OD_TOfail", "OD_SLS"]  # , 'OD_TO', 'OD_TOC', 'OD_LDG']
...
if __name__ == "__main__":
    ...
    bal.add_balance('OD_SLS_Fn_target', val=45000.0, units='lbf', eq_units='lbf', use_mult=True, mult_val=1.2553, ref0=45000.0, ref=55000.0)
    ...
    prob.set_val("fan:PRdes", 1.4)
    prob.set_val("lpc:PRdes", 1.7)
    prob.set_val('DESIGN.balance.rhs:hpc_PR', 39)
    ...
    FAR_guess = [0.03081, 0.03041]
    W_guess = [1503.771, 1700.]
    BPR_guess = [8.4, 8.0]
    lp_Nmech_guess = [10000, 10000]
    hp_Nmech_guess = [14217, 15000.1]
    hpt_PR_guess = [4.256, 4.197]
    lpt_PR_guess = [5.051, 10.803]
    fan_Rline_guess = [2.606, 2.397]
    lpc_Rline_guess = [2.0052, 2.1075]
    hpc_Rline_guess = [2.116, 2.1046]
