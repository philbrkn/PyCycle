...
class HBTF(pyc.Cycle):
    ...
    def setup(self):
        ...
        if design:
            ...
            balance.add_balance('BPR', lower=1., upper=10., eq_units='inch**2')
            self.connect('balance.BPR', 'splitter.BPR')
            self.connect('byp_nozz.Throat:stat:area', 'balance.lhs:BPR')
...
if __name__ == "__main__":
    ...
    prob.set_val('DESIGN.lpc.PR', 3.5)
    prob.set_val('DESIGN.hpc.PR', 11)
    prob.set_val('DESIGN.splitter.BPR', 2.9)
    ...
    for pt in ['OD_TOfail']: #, 'OD_TO']: #, 'OD_TOC', 'OD_LDG']:
        ...
        prob[pt+'.balance.W'] = 900
        prob[pt+'.balance.BPR'] = 2.9
