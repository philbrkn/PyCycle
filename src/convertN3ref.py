...
class N3(pyc.Cycle):
    ...
    def setup(self):
        ...
        main_order = ['fc', 'inlet', 'fan', 'splitter', 'duct2', 'lpc', 'bld25', 'duct25', 'hpc', 'bld3', 'burner', 'hpt', 'duct45',
                            'lpt', 'duct5', 'core_nozz', 'byp_bld', 'duct17', 'byp_nozz', 'lp_shaft', 'hp_shaft',
                            'perf', 'ext_ratio']
        ...
        if design:
            ...
            self.add_subsystem('opr_calc', om.ExecComp('OPR_simple = FPR*LPCPR*HPCPR',
                            FPR={'val':1.7, 'units':None},
                            LPCPR={'val':1.20, 'units':None},
                            HPCPR={'val':16.0, 'units':None},
                            OPR_simple={'val':34.8, 'units':None}))
            ...
            balance.add_balance('BPR', lower=2., upper=12.)
            self.connect('balance.BPR', 'splitter.BPR')
            self.connect('fan.map.RlineMap', 'balance.lhs:BPR')


            balance.add_balance('lp_Nmech', val=5000.0, units='rpm', lower=500., eq_units='hp', rhs_val=0., res_ref=1e2)
            self.connect('balance.lp_Nmech', 'LP_Nmech')
            self.connect('lp_shaft.pwr_net', 'balance.lhs:lp_Nmech')

            balance.add_balance('hp_Nmech', val=15000.0, units='rpm', lower=500., eq_units='hp', rhs_val=0., res_ref=1e2)
...
class MPN3(pyc.MPCycle):
    ...
    def setup(self):
        ...
        # POINT 1: Top-of-climb (CRZ)
        self.set_input_defaults('CRZ.fc.alt', 28000., units='ft'),
        self.set_input_defaults('CRZ.fc.MN', 0.74),
        self.set_input_defaults('CRZ.inlet.ram_recovery', 0.9980),

        # self.set_input_defaults('CRZ.balance.rhs:fan_eff', 0.97)
        self.set_input_defaults('CRZ.duct2.dPqP', 0.0100)
        # self.set_input_defaults('CRZ.balance.rhs:lpc_eff', 0.905),
        self.set_input_defaults('CRZ.duct25.dPqP', 0.0150),
        # self.set_input_defaults('CRZ.balance.rhs:hpt_eff', 0.91),
        self.set_input_defaults('CRZ.duct45.dPqP', 0.0050),
        # self.set_input_defaults('CRZ.balance.rhs:lpt_eff', 0.92),
        self.set_input_defaults('CRZ.duct5.dPqP', 0.0100),
        self.set_input_defaults('CRZ.duct17.dPqP', 0.0150),
        self.set_input_defaults('CRZ.LP_Nmech', 5772.0, units='rpm'),
        self.set_input_defaults('CRZ.HP_Nmech', 15871.0, units='rpm'),

        self.set_input_defaults('CRZ.inlet.MN', 0.625),
        ...
        self.set_input_defaults('CRZ.duct2.MN', 0.45),
        self.set_input_defaults('CRZ.lpc.MN', 0.45),
        self.set_input_defaults('CRZ.bld25.MN', 0.45),
        self.set_input_defaults('CRZ.duct25.MN', 0.45),
        ...
        self.set_input_defaults('CRZ.duct45.MN', 0.45),
        self.set_input_defaults('CRZ.lpt.MN', 0.35),
        self.set_input_defaults('CRZ.duct5.MN', 0.25),
        self.set_input_defaults('CRZ.byp_bld.MN', 0.45),
        self.set_input_defaults('CRZ.duct17.MN', 0.45),
        ...
        # self.od_MNs = [0.18, 0.000001, 0.74]
        # self.od_alts = [0.0, 0.0, 28000.0]
        # self.od_dTs = [27.0, 27.0, 0.0]
        # self.od_BPRs = [1.75, 1.75, 1.9397]  # CHXNGE[]
        ...
        # self.set_input_defaults('RTO.balance.rhs:FAR', 45000.0, units='lbf'),
        ...
        #                                  CRZ_T4={'val': 2880.0, 'units':'degR'},
        #                                  TR={'val': 0.84706, 'units': None}), promotes_inputs=['RTO_T4',])
...
def N3ref_model():
    ...
    prob.model.add_design_var('CRZ.balance.rhs:hpc_PR', lower=30.0, upper=50.0, ref0=30.0, ref=50.0)
...
if __name__ == "__main__":
    ...
    prob.set_val('CRZ.fc.W', 650, units='lbm/s')
    prob.set_val('CRZ.splitter.BPR', 7.9),
    prob.set_val('CRZ.balance.rhs:hpc_PR', 34)

    # Set up the specific cycle parameters
    prob.set_val('fan:PRdes', 1.500),
    prob.set_val('lpc:PRdes', 1.200),
    # prob.set_val('T4_ratio.TR', 0.84706)
    # prob.set_val('RTO_T4', 3400.0, units='degR')
    # prob.set_val('SLS.balance.rhs:FAR', 50000.0, units='lbf')
    # prob.set_val('RTO.balance.rhs:FAR', 45000.0, units='lbf')
    # prob.set_val('ToC.balance.rhs:FAR', 12000.0, units='lbf')
    # prob.set_val('RTO.hpt_cooling.x_factor', 0.9)

    # Set initial guesses for balances
    prob['CRZ.balance.FAR'] = 0.0250
    prob['CRZ.balance.lpt_PR'] = 8.937
    prob['CRZ.balance.hpt_PR'] = 4.185
    prob['CRZ.fc.balance.Pt'] = 5.272
    prob['CRZ.fc.balance.Tt'] = 444.41

    # od points: [RTO, SLS, ToC]
    # FAR_guess = [0.02832, 0.02541, 0.02510]
    # W_guess = [1916.13, 1900., 802.79]
    # BPR_guess = [5.5620, 5.3467, 5.2233]
    # lp_Nmech_guess = [6611.2, 6054.5, 6567.9]
    # hp_Nmech_guess = [16288.2, 16594.0, 16574.1]
    ...
    viewer_file = open(f"output_data/n3_hbtf2_{date_time}.out", "w")
